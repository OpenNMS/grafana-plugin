...
| nanToZero
| Convert all `NaN` values to zero (`0`) so they can be taken into consideration by reduce expressions (`sum`, `max`).
|===
...
You can use this inside a `exporterNodesWithFlows()` query to filter the number of nodes displayed by location.
