...
In addition, `labelFormat` and `valueFormat` attributes can be specified.
Please refer to the <<datasources:entity_datasource.adoc#ds-nodefilter-label-value-formats, Entity Datasource documentation>> for more information.
If not specified, the default for `labelFormat` is `label` and the default for `value` is `fs:fid` if available, otherwise the Node `id.`
