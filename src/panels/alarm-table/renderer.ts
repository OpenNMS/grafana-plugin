...
export class TableRenderer {
  ...
  /** @ngInject */
  constructor(public panel: any, public table: TableModel, public isUtc: boolean, public sanitize: Function, public selectionMgr: SelectionMgr, public templateSrv: any, public theme: any) {
    this.initColumns();
  }
  ...
  createColumnFormatter(column) {
    ...
    if (column.style.type === 'date') {
      return v => {
        ...
        if (!isNaN(v) && _.isString(v) && v.length > 12) {
          v = parseInt(v, 10);
        }
  ...
  renderCell(columnIndex: number, rowIndex: number, value: any, addWidthHack: boolean, columnClasses: string[]) {
    const title = !_.isNil(value) && _.isString(value) ? ' title="' + value.trim().replace(/"/g, '&quot;') + '"' : '';

    value = this.formatColumnValue(columnIndex, value);

    const column = this.table.columns[columnIndex];
    const cellStyles = [] as string[];
    let cellStyle = '';
    const textStyles = [] as string[];
    let textStyle = '';
    const cellClasses = [...columnClasses];
    let cellClass = '';
  ...
  render(page) {
    ...
    for (let y = startPos; y < endPos; y++) {
      ...
      const severity = alarm && alarm.severity && alarm.severity.label ? alarm.severity.label.toLowerCase() as string : undefined;

      for (let i = 0; i < this.table.columns.length; i++) {
        let columnClasses = [] as string[];
        ...
        if (col.style.type) {
          if (severity && col.style.type === 'severity' && this.panel.severity === 'column') {
      ...
      if (severity && (this.panel.severity === true || this.panel.severity === 'row')) {
