...
const CORRECT_OUTLINED = "btn feedback-button feedback-button";
const CORRECT_FILLED = "btn feedback-button feedback-button-correct";

const INCORRECT_OUTLINED = "btn feedback-button feedback-button";
const INCORRECT_FILLED = "btn feedback-button feedback-button-incorrect";

const ROOT_CAUSE_NO = "btn feedback-button feedback-button";
const ROOT_CAUSE_YES = "btn feedback-button feedback-button-root-cause";
...
export class AlarmDetailsCtrl {
  ...
  /** @ngInject */
  constructor(public $scope: any, public backendSrv: any, public contextSrv: any, public datasourceSrv: any) {
    ...
    $scope.feebackButton = CORRECT_OUTLINED;
  ...
  detailFeedbackOkayButton(reductionKey) {
    let button = CORRECT_FILLED;
    if (this.$scope.situationFeedback) {
      for (let feedback of this.$scope.situationFeedback) {
        if (feedback.alarmKey === reductionKey && feedback.feedbackType.id === Model.FeedbackTypes.FALSE_POSITIVE.id) {
          button = CORRECT_OUTLINED;
          break;
        }
      }
    }
  ...
  detailFeedbackRootCauseButton(reductionKey) {
    let button = ROOT_CAUSE_NO;
    if (this.isRootCause(reductionKey)) {
      button = ROOT_CAUSE_YES;
    }
    return button;
  }
  ...
  initalizeFeeback() {
    ...
    let feedback = [] as Model.OnmsSituationFeedback[];
