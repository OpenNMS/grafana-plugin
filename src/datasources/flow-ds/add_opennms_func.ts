          ...
          ($input as any).typeahead({
            ...
            updater: function (value) {
              ...
              if (!funcDef) {
                ...
                funcDef = _.find(allFunctions, function (funcName: string) {
...
function getAllFunctionNames(categories): string[] {
  return _.reduce(categories, function (list: any[], category: any[]) {
    _.each(category, function (func: { name: string }) {
      list.push(func.name);
    });
    return list;
  }, []);
...
function createFunctionDropDownMenu(categories, selectedSegment, selectedFunctions) {
  let categoriesToRender = [] as any[];

  _.forEach(categories, (functionsInCategory, categoryName) => {
    let functionsToRender = [] as any[];
