...
export default class AlarmEntity extends Entity {
      ...
      filterPanel.columns.forEach((column) => {
        ...
        if (selected.value) {
          ...
          if (values.length === 0) {
            ...
            values.forEach(val => {
              if (val) {
                restriction.withOrRestriction(getValueRestriction(val));
              }
            });
  ...
  static toTable(alarms: any[], columns: any[], metadata, datasourceName) {
    const mapped = _.map(alarms, (alarm) => {
    ...
    let parameterNames = _.uniq(_.sortBy(_.flatten(mapped), name => name));
