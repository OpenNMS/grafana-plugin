...
System.register(['../datasources/perf-ds/interpolate'], function (_export, _context) {
  "use strict";

  var interpolate;
  return {
    setters: [function (_datasourcesPerfDsInterpolate) {
      interpolate = _datasourcesPerfDsInterpolate.interpolate;
    }],
    execute: function () {
      ...
      describe('OpenNMSPMDatasource :: interpolate', function () {
        ...
        it('should be able to interpolate multiple variables in a single attribute', function () {
          var interpolated = interpolate(query, ['metric'], [{ name: 'x', value: ['x1', 'x2'] }, { name: 'y', value: ['y1', 'y2'] }]);
        ...
        it('should be able to interpolate multiple variables in multiple attributes', function () {
          var interpolated = interpolate(query, ['resource', 'metric'], [{ name: 'node', value: ['1', '2'] }, { name: 'x', value: ['x1', 'x2'] }, { name: 'y', value: ['y1', 'y2'] }]);
        ...
        it('should support interpolating a special variable named $index which is unique for every row', function () {
          ...
          var interpolated = interpolate(queryWithIndex, ['resource', 'metric', 'label'], [{ name: 'x', value: ['x1', 'x2'] }, { name: 'y', value: ['y1', 'y2'] }]);
