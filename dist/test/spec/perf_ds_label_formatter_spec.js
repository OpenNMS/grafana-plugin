...
var _function_formatter = require('../datasources/perf-ds/function_formatter');
...
describe('OpenNMSPMDatasource :: LabelFormatter', function () {
    ...
    describe('findFunctions()', function () {
        ...
        it('find a simple function with one argument', function () {
            var found = _function_formatter.FunctionFormatter.findFunctions('simpleFunction(foo)');
            expect(found).to.exist;
            expect(found.length).to.equal(1);
            expect(found).to.deep.equal([{
                name: 'simpleFunction',
        ...
        it('find a simple function with many arguments', function () {
            var found = _function_formatter.FunctionFormatter.findFunctions('simpleFunction(foo, bar, baz)');
            expect(found).to.exist;
            expect(found.length).to.equal(1);
            expect(found).to.deep.equal([{
                name: 'simpleFunction',
        ...
        it('find multiple functions with arguments', function () {
            var found = _function_formatter.FunctionFormatter.findFunctions('simpleFunction(foo, bar, baz), anotherThing($nodes)');
            expect(found).to.exist;
    ...
    describe('replace()', function () {
        it('do not replace an unmatched function', function () {
            var res = _function_formatter.FunctionFormatter.replace('unmatchedFunction()', {
        ...
        it('replace a simple function without arguments', function () {
            var res = _function_formatter.FunctionFormatter.replace('simpleFunction()', {
                simpleFunction: function simpleFunction() {
                    return 'foo';
                }
            });
        ...
        it('replace a simple function with an argument', function () {
            var res = _function_formatter.FunctionFormatter.replace('exclamationer(foo)', FUNCS);
            expect(res).to.equal('foo!!!');
        });
        it('replace a simple function with multiple arguments', function () {
            var res = _function_formatter.FunctionFormatter.replace('insulter(Bob, jerk)', FUNCS);
    ...
    describe('format()', function () {
        it('nodeToLabel(foreignSource:foreignId)', function () {
            var res = _function_formatter.FunctionFormatter.format('nodeToLabel(situation-test:nodea)', metadata);
            expect(res).to.equal('ThisIsAVeryLongNodeLabel1');
        });
        it('nodeToLabel(nodeId)', function () {
            var res = _function_formatter.FunctionFormatter.format('nodeToLabel(1)', metadata);
            expect(res).to.equal('ThisIsAVeryLongNodeLabel1');
        });
        it('nodeToLabel(invalid)', function () {
            var res = _function_formatter.FunctionFormatter.format('nodeToLabel(3)', metadata);
            expect(res).to.equal('3');
        });

        it('resourceToLabel(resourceId)', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToLabel(node[situation-test:nodea].responseTime[127.0.0.1])', metadata);
            expect(res).to.equal('127.0.0.1');
        });
        it('resourceToLabel(situation-test:nodea, responseTime[127.0.0.1])', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToLabel(situation-test:nodea, responseTime[127.0.0.1])', metadata);
            expect(res).to.equal('127.0.0.1');
        });
        it('resourceToLabel(invalid)', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToLabel(foo)', metadata);
            expect(res).to.equal('foo');
        });
        it('resourceToLabel(invalid, invalid)', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToLabel(foo, bar)', metadata);
            expect(res).to.equal('foo.bar');
        });

        it('resourceToName(resourceId)', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToName(node[situation-test:nodea].responseTime[127.0.0.1])', metadata);
            expect(res).to.equal('localhost');
        });
        it('resourceToName(situation-test:nodea, responseTime[127.0.0.1])', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToName(situation-test:nodea, responseTime[127.0.0.1])', metadata);
            expect(res).to.equal('localhost');
        });
        it('resourceToName(invalid)', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToName(foo)', metadata);
            expect(res).to.equal('foo');
        });
        it('resourceToName(invalid, invalid)', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToName(foo, bar)', metadata);
            expect(res).to.equal('foo.bar');
        });

        it('resourceToInterface(resourceId)', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToInterface(node[situation-test:nodeb].interfaceSnmp[en0-000123456789])', metadata);
            expect(res).to.equal('en0');
        });
        it('resourceToInterface(situation-test:nodeb, interfaceSnmp[en0-000123456789])', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToInterface(situation-test:nodeb, interfaceSnmp[en0-000123456789])', metadata);
            expect(res).to.equal('en0');
        });
        it('resourceToInterface(invalid)', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToInterface(foo)', metadata);
            expect(res).to.equal('foo');
        });
        it('resourceToInterface(invalid, invalid)', function () {
            var res = _function_formatter.FunctionFormatter.format('resourceToInterface(foo, bar)', metadata);
            expect(res).to.equal('foo.bar');
        });
    });
});
