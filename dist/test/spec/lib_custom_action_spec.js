...
describe('CustomAction :: getMatch', function () {
  it('should find an exact match', function () {
    var match = (0, _custom_action.getMatch)('$foo', 'foo');
  ...
  it('should find a substring match', function () {
    var match = (0, _custom_action.getMatch)('I like $foo bar!', 'foo');
    expect(match).to.be.defined;
    expect(match.token).to.equal('$foo');
    expect(match.variable).to.equal('foo');
    expect(match.index).to.be.undefined;
  ...
  it('should not find a substring match without a word boundary', function () {
    expect((0, _custom_action.getMatch)('I like $foobar!', 'foo')).to.be.null;
  });

  it('should find a match with a numerical index inside brackets', function () {
    var match = (0, _custom_action.getMatch)('I like $foo[1]', 'foo');
    expect(match).to.be.defined;
  ...
  it('should find a match with a string index inside brackets', function () {
    var match = (0, _custom_action.getMatch)('I like $foo[blah]', 'foo');
    expect(match).to.be.defined;
  ...
  it('should find a match with empty brackets', function () {
    var match = (0, _custom_action.getMatch)('I like $foo[]', 'foo');
    expect(match).to.be.defined;
    expect(match.token).to.equal('$foo[]');
    expect(match.variable).to.equal('foo');
    expect(match.index).to.be.undefined;
  ...
  */
  ...
  describe('constructor', function () {
    ...
    it('should validate a url with no variables', function () {
      var ca = new _custom_action.CustomAction('foo', 'http://bar/');
    ...
    it('should validate a url with a nodeLabel variable', function () {
      var ca = new _custom_action.CustomAction('foo', 'http://bar/$nodeLabel');
      var alarm = new _opennms.Model.OnmsAlarm();
      alarm.nodeId = 5;
    ...
    it('should validate a variable that refers to an object', function () {
      var ca = new _custom_action.CustomAction('foo', 'http://bar/$severity');
      var alarm = new _opennms.Model.OnmsAlarm();
      alarm.severity = _opennms.Model.Severities.NORMAL;
      expect(ca.validate(alarm)).to.be.true;
    });
    ...
    it('should validate a variable with a number index', function () {
      var ca = new _custom_action.CustomAction('foo', 'http://bar/$parameters[0]');
      var alarm = new _opennms.Model.OnmsAlarm();
      alarm.parameters = [new _opennms.Model.OnmsParm('blah', 'string', 'yo')];
      expect(ca.validate(alarm)).to.be.true;
    });
    ...
    it('should validate a variable with a string index', function () {
      var ca = new _custom_action.CustomAction('foo', 'http://bar/$blah[monkey]');
      var obj = {
        blah: {
          monkey: 'see'
        }
    ...
    it('should validate a variable with a named event parm', function () {
      var ca = new _custom_action.CustomAction('foo', 'http://bar/$parameters[monkey]');
      var obj = new _opennms.Model.OnmsAlarm();
      obj.parameters = [new _opennms.Model.OnmsParm('monkey', 'Int32', '6')];
      expect(ca.validate(obj)).to.be.true;
    });
