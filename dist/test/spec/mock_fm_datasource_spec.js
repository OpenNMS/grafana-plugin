...
var _module = require("../datasources/mock-fault-ds/module");
...
describe('OpenNMS_Mock_FaultManagement_Datasource', function () {
  ...
  beforeEach(function () {
    ...
    ctx.backendSrv = {};
    ctx.templateSrv = {};
    ctx.ds = new _module.Datasource({ 'name': 'mock' }, ctx.$q, ctx.backendSrv, ctx.templateSrv);
  });
  ...
  it('should include meta-data in the rows', function (done) {
    ...
    ctx.ds.query({ targets: ['foo'] }).then(function (result) {
      expect(result.data).to.have.length(1);
      expect(result.data[0].rows).to.have.length(1);
      _lodash2.default.each(result.data[0].rows, function (row) {
        // Datsource name
