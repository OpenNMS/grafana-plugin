...
function cartesianProductOfVariables(variables) {
  ...
  _lodash2.default.each(productOfAllValues, function (rowOfValues) {
    var rowOfVariables = [];
...
function defaultContainsVariable(value, variableName) {
  if (_lodash2.default.isNull(value) || _lodash2.default.isEmpty(value)) {
    return false;
  }
...
function interpolate(object, attributes, variables, callback, containsVariable, replace) {
  ...
  var variablesWithIndex = _lodash2.default.clone(variables);
  ...
  _lodash2.default.each(variablesWithIndex, function (variable) {
    var isVariableReferenced = _lodash2.default.find(attributes, function (attribute) {
  ...
  var objects = [];
  var index = 0;
  _lodash2.default.each(productOfAllVariables, function (rowOfReferencedVariables) {
    // Update the value of the index variable to reflect the index of the row
    _lodash2.default.each(rowOfReferencedVariables, function (variable) {
    ...
    var o = _lodash2.default.clone(object);
